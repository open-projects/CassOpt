...
def main():
    ...
    cursor.execute('''INSERT INTO stealth_junctions (hla, l_name, r_name, l_pos, r_pos)
                   ...
                   GROUP BY l_name, r_name, l_pos, r_pos, hla
    ...
    cursor.execute('''SELECT l_name, MAX(l_pos) FROM stealth_junctions GROUP BY l_name''')
    name_len = {}
    for name, max_len in cursor.fetchall():
        name_len[name] = max_len
    
    print("permutations: ", len(permutation), file=sys.stderr)
    
    cassettes = []
    for p, names in enumerate(permutation):
        paths = []
        hla_path = set(hla_set)
        for i in range(len(names) - 1):
            l_name, r_name =  names[i], names[i + 1]
            sql_query = "SELECT COUNT(hla) AS n, l_pos, r_pos FROM stealth_junctions WHERE l_name = ? AND r_name = ? AND hla IN ({}) GROUP BY l_pos, r_pos ORDER BY n DESC".format(",".join("'"+ x + "'" for x in hla_path))
            cursor.execute(sql_query, (l_name, r_name))
            
            n_max = 0
            pos_set = []
            for n, l_pos, r_pos in cursor.fetchall():
                if n_max <= n:
                    n_max = n
                    pos_set.append((l_pos, r_pos))
                    cursor.execute('''SELECT DISTINCT hla
                            FROM stealth_junctions
                            WHERE l_name = ? AND r_name = ? AND l_pos = ? AND r_pos = ?''', (l_name, r_name, l_pos, r_pos))
                    hla_path = set()
                    for hla in cursor.fetchall():
                        hla_path.add(hla[0])
                else:
                    break
            
            path_ext = []
            for l_pos, r_pos in pos_set:
                if i == 0:
            ...
            print("permutation: {}, hop: {}, total paths: {}".format(p, i, len(path_ext)), file=sys.stderr)
            paths = path_ext
            if len(paths) == 0:
                break
        if len(paths):
        ...
        for path in cassettes:
            ...
            for item in path.get():
                if len(casstte_path):
                    ...
                    if item['l_name'] in name_len:
                        total_spared_len += name_len[item['l_name']] - item['l_pos']
                    total_spared_len += item['r_pos'] - 1
                else:
                    casstte_path = item['l_name'] + '>' + str(item['l_pos']) + '|' + str(item['r_pos']) + '<' + item['r_name']
            print(str(total_spared_len) + "\t" + str(len(hla_path)) + "\t" + ";".join(hla_path) + "\t" + casstte_path)
# end of main()
